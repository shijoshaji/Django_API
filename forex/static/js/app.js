...
const API_KEY = '07c7436812f210f455d48d84';
const END_POINT = 'latest';
const BASE_URI = 'https://v6.exchangerate-api.com/v6/';

// Fetch exchange rate from the API
function calculate() {
  const curr1 = c1.value;
  const curr2 = c2.value;
  fetch(`${BASE_URI}${API_KEY}/${END_POINT}/${curr1}`)
    // fetch(`${BASE_URI}${END_POINT}?access_key=${API_KEY}`)
    .then((res) => res.json())
    .then((data) => {
      console.log(data);
      // Updating the data
